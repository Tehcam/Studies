...
import com.example.socketservice.MainService;
import com.example.socketservice.MySocket;
...
public class BarcodeScannerProcessor extends VisionProcessorBase<List<Barcode>> {
  ...
  protected void onSuccess(
    ...
    for (int i = 0; i < barcodes.size(); ++i) {
      ...
      //!\\ DU CODE EN PLUS POUR LE RETOUR DU TRAITEMENT VERS LA SOCKET //!\\

      MySocket socket = MainService.getMySocket();
      if(socket != null) {
        Thread thread = new Thread(() -> socket.getProperty().setBarcode(barcode));
        thread.start();
      }
